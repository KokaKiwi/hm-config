...
let
  bashCompletion = fetchFromGitHub {
    ...
    hash = "sha256-9rHk/Ps6+TT57kUaHHMLLTCGFy7pwMafVST2tEPkw3A=";
  };
in stdenv.mkDerivation (finalAttrs: {
  ...
  ++ lib.optionals withJemalloc [ jemalloc ];

  configureFlags = with lib.strings; [
    "--sysconfdir=/etc"
    "--localstatedir=/var"

    (enableFeature withSystemd "systemd")
    (enableFeature withSixel "sixel")
    (enableFeature withUtempter "utempter")
    (enableFeature withUtf8proc "utf8proc")
  ...
  passthru = {
    terminfo = runCommand "tmux-terminfo" {
      nativeBuildInputs = [
        ncurses
      ];
